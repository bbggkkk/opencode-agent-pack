...
초고 파이프라인은 씨앗에서 열매까지 자라는 서사 성장 모델을 따릅니다. 사용자의 요청은 **씨앗**입니다. 라우터는 그 씨앗에서 **가지**를 먼저 키워 Macro Skeleton, Length Budget, Execution Unit Queue를 만들거나 사용자가 준 큰 흐름을 채택합니다. 각 가지에는 목표/최소 분량도 함께 붙습니다. 기존 피드백 루프는 각 단위를 목표 분량에 맞춰 실제 문장으로 붙이는 **잎** 단계이며, 편집자의 미시적 문장 정제와 설정 검증가의 엄격한 거시 흐름 및 분량 검증으로 정교화하는 과정은 **꽃** 단계입니다. **열매**는 요청 범위 완료, 약속 분량 충족, 최종 설정 검증 통과, 문체/어투 감사 통과, 서사 원장, 검증 목록, 검증 증거, 커밋까지 끝난 검증 완료 산출물입니다.
...
```
   ...
   ② 설정 기록가 → 설정 데이터 및 최근 서사 상태 수집 (설정만 수집, 문체는 관여 안 함)
         │
   ③ 가지 → Macro Skeleton, Length Budget, Execution Unit Queue 작성
         │
   ┌─────►④ 잎/꽃 루프: 각 실행 단위에 대해:
   │      │
...
- **씨앗-열매 서사 성장**: 초고는 고립된 문장부터 시작하지 않습니다. 라우터는 먼저 씨앗을 기록하고, Length Budget이 붙은 Macro Skeleton이라는 가지를 만들고, Execution Unit Queue를 세운 뒤, 기존 피드백 루프로 잎을 붙이고, 검토로 꽃을 피우며, 검증과 산출물 갱신이 끝난 뒤에만 열매를 전달합니다.
...
- **분량 예산**: 가지와 실행 단위에는 처음부터 목표/최소 글자 수가 붙습니다. 라우터는 통합 뒤 정본 원고 파일에서 실제 글자 수를 세어 갱신하며, 어떤 단위, 가지, 장, 권, 작품도 `UNDER_LENGTH` 상태에서는 완료될 수 없습니다.
