...
## 저작권 및 스타일 카피 방침 (Copyright & Style Policy)

에이전트는 실존하는 작가의 문체나 저작권이 있는 특정 음악의 가사를 직접 모방하지 않습니다. 대신 대안적인 창작 분위기, 장르 특성, 템포, 서사 구조, 정서적 흐름, 고유 이미지 등의 Craft 언어를 사용하여 대체합니다.

올바른 요청 예시:
```text
어두운 도시 판타지 스타일의 1장 오프닝 씬을 작성해 줘.
90년대 발라드 스타일의 이별 후렴구 가사를 써줘.
강렬한 훅을 가진 K-pop 댄스곡 가사를 써줘.
```

지양해야 하는 요청 예시:
```text
[특정 소설가]의 문체와 완벽하게 똑같이 써줘.
[특정 대중가요] 가사와 똑같은 분위기로 만들어 줘.
이 기존 가사에서 몇 단어만 살짝 바꿔 줘.
```
